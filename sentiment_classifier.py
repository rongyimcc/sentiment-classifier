...
def build_vocab(Xt, min_freq=1):
    ...
    end = len(sorted_token_freq_pairs)
    for i, (token, freq) in enumerate(sorted_token_freq_pairs):
        if freq < min_freq:
            end = i
            break
...
class SentimentClassifier(nn.Module):
    ...
    def __init__(self, vocab_size, emb_size=128, ffn_size=128, num_tfm_layer=2, num_head=2, p_dropout=0.2, max_seq_len=300):
        ...
        self.transformer = nn.TransformerEncoder(self.encoder_layer, num_layers=num_tfm_layer)
    ...
    def forward(self, x):
        ...
        emb = self.token_embeddings(x)  # [seq_len, batch_size, emb_size]
        seq_len = emb.size(0)

        emb = emb + self.positional_encoding[:seq_len, :].unsqueeze(1)

    # dropout
        emb = self.dropout(emb)

    # Transformer Encoder
        out = self.transformer(emb)  # [seq_len, batch_size, emb_size]

        cls_out = out[0, :, :]  # [batch_size, emb_size]

        logits = self.linear(cls_out).squeeze(1)
        return logits
...
def train_model(model, dataset_train, dataset_val, batch_size=64, num_epoch=1, learning_rate=0.001, fmodel='best_model.pth'):
    ...
    for epoch in range(num_epoch):
        running_loss = 0.0
        for i, (Xb, yb) in enumerate(dataloader_train):
            logits = model(Xb)
            ...
            if (i + 1) % 10 == 0 or (i + 1) == len(dataloader_train):
                avg_loss = running_loss / (i + 1)
                print(f'{epoch+1:>10} {i+1:>10} {avg_loss:>20.4f}')
        
        # Evaluate after each epoch
        val_acc = eval_model(model, dataset_val)
        print(f'Epoch {epoch+1}/{num_epoch}, Validation Accuracy: {val_acc:.4f}')
        if val_acc > best_acc:
            best_acc = val_acc
